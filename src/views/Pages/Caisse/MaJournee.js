...
class MaJournee extends Component {
    ...
    componentDidMount(){
        ...
        const apiUrl = 'http://localhost:8080/api/soldeDebuterJournees/caissier/'+idU;
        fetch(apiUrl)
...
class ModifierDebuter extends Component{
    ...
    var idU=JSON.parse(localStorage.getItem('logaccorde'))[0].id;
        axios.put('http://localhost:8080/api/soldeDebuterJournees/modifier/'+this.journee().id,{caisse:$('#caisse').val(),orange:$('#orange').val(),expresso:$('#expresso').val(),freeMoney:$('#free').val(),wari:$('#wari').val(),wizall:$('#wizall').val(),proximo:$('#proximo').val(),xpress:$('#xpress').val(),yup:$('#yup').val(),wave:$('#wave').val(),cloturer:0,sommeInitiale:somme,date:NewDate,idU:idU})
